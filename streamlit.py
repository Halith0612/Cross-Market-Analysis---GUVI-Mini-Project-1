...
if selection == "Market Overview":
    ...
    stks["date"] = pd.to_datetime(stks["date"], errors="coerce")
    ...
    cols = [col for col in ["date","price_btc","price_oil","close_nifty","close_sp500"] if col in snapshot.columns] # Ensure only existing columns are selected for display
    st.dataframe(snapshot[cols])    

elif selection == "SQL Query Runner":
